...
<%@ include file="/WEB-INF/views/layout/header.jsp" %>
	...
	<input type="hidden" value="${principal.id }" id="user_id">
	...
	<ul>
		<li>
			<div class="order_info">
				...
				<ul>
					<c:set var="cart" value="${cartList.cartDto }" />
		...
		<li class="delivery_cont">
		
			<div class="delivery_info">
				<h2>배달정보 </h2>
				<div>
					<span>주소 :</span> 
					<span class="address1"><input id="address1" value="" readonly></span> 
					<button type="button" onclick="execution_daum_address()" id="delivery_modify" >주소 변경하기</button>
	             	<%-- <%@ include file="/WEB-INF/views/layout/modifyAddress.jsp" %> --%>
				</div>
				
				<input type="hidden" id="deliveryAddress1" value="" name="deliveryAddress1"> 
				<input type="hidden" id="deliveryAddress2" value="" name="deliveryAddress2"> 
				
				
				<div>상세 주소</div>
				<div class="input_area"><input type="text" id="deliveryAddress3" maxlength="100" value=""  name="deliveryAddress3"> </div>
				 
				<div>전화번호</div>
				<c:if test="${!empty principal  }">
					<div class="input_area"> <input type="number" value="${principal.phone }" name="phone" readonly required onkeypress="return lenthCheck(this, 11)" autocomplete="off" > </div>
				</c:if>
				<c:if test="${empty principal.phone || empty principal  }">
		...
		<li class="point_area">
			...
			<div class="point">
				<div class="point_click">
					<c:if test="${!empty principal  }">
						<span><fm:formatNumber value="${principal.point }"  pattern="###,###" />원 사용 가능</span>
						<input type="hidden" value="${principal.point }" id="point">
					</c:if>
		...
		<li class="pay">
			...
			<div>배달팁 <fm:formatNumber value="${cartList.deliveryTip }"  pattern="###,###" />원 </div> 
			
			<div class="point_dis"><span>포인트 할인 </span><span></span> </div>
				
			<div class="total">
				<fm:formatNumber value="${cartList.cartTotal + cartList.deliveryTip}"  pattern="###,###" />원 결제하기
			</div>
			
			<input type="hidden" value="${cartList.cartTotal + cartList.deliveryTip}" name="total" id="total"> 
			<input type="hidden" value="${cartList.deliveryTip }" name="deliveryTip" id="delivery_tip"> 
